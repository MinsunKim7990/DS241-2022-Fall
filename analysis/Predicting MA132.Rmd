...
df <- df %>% filter(component == "LEC", course_id %in% c(8732, 8733), startsWith(term_8, "Fall") | startsWith(term_8, "Spring")) %>%
  select(course_id, catalog, term_7, term_8, title, tot_enrl, section)
```

```{r}
df_ma132_s = df %>% filter(course_id == 8733, startsWith(term_8, "Spring")) %>% 
  group_by(term_8) %>%
